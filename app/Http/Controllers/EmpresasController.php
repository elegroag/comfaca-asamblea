...
class EmpresasController extends Controller
{
    ...
    public function index()
    {
        ...
        $representante = DB::table('asa_representantes')
            ->select('asa_representantes.cedrep', 'empresas.repleg')
            ->leftJoin('empresas', 'empresas.cedrep', '=', 'asa_representantes.cedrep')
            ->where('asa_representantes.cedrep', $cedrep)
            ->first();

        $empresas = $this->loadTable($this->idAsamblea, $cedrep);
    ...
    /**
     * Vista de edición de empresa
     */
    public function editar_view($nit)
    {
        $autenticacionCheck = $this->verificarAutenticacion();
        if ($autenticacionCheck) return $autenticacionCheck;
        ...
        $empresa = DB::table('empresas')
            ->select(
                ...
                DB::raw("(SELECT count(*) FROM registro_ingresos
                    WHERE registro_ingresos.nit = empresas.nit AND registro_ingresos.asamblea_id='{$this->idAsamblea}') AS 'has_asistencia'")
            )
            ->where('nit', $nit)
            ->first();

        $ingresos = DB::table('registro_ingresos')
        ...
        $asistencia = RegistroIngresos::where('nit', $nit)
            ->where('asamblea_id', $this->idAsamblea)
            ->where('cedula_representa', $empresa->cedrep)
            ->first();

        $empresaData = $empresa ? $empresa->toArray() : [];
        $empresaData['tipo_ingreso'] = $asistencia ? $asistencia->tipo_ingreso : "";
        $empresaData['asistencia_estado'] = $asistencia ? $asistencia->estado : "";
        $empresaData['asistencia_estado_detalle'] = $asistencia ? $asistencia->estado_detalle : "";
        $empresaData['cantidad_ingresos'] = $ingresos->cantidad ?? 0;
        $empresaData['asistencias'] = $this->asistencias;

        return view('empresas.editar', $empresaData);
    }
    ...
    public function poderes()
    {
        ...
        return view('empresas.poderes', [
            'poderes_poderdante' => $this->loadPoderesPoderdante($cedrep),
            'poder_apoderado' => $this->loadPoderesApoderado($cedrep),
            'titulo' => 'Gestión Poderes'
        ]);
    }
